// MISMO EJERCICIO UTILIZANDO LOS MÉTODOS .REDUCE Y .FILTER
...
function findOldestXMen(xMen) {
  const oldestYear = xMen.reduce((minYear, member) => {
    if (member.year < minYear) {
      return member.year
    } else {
      return minYear
    }
  ...
  return xMen.filter((member) => member.year === oldestYear)
}
